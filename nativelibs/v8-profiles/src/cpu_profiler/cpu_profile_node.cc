...
Local<Value> ProfileNode::GetLineTicks_(v8::Isolate* isolate,
  ...
  v8::CpuProfileNode::LineTick* entries =
      new v8::CpuProfileNode::LineTick[count];
  ...
  if (result) {
    ...
    for (uint32_t index = 0; index < count; index++) {
      ...
      Nan::Set(tick, Nan::New<String>("line").ToLocalChecked(),
               Nan::New<Integer>(entries[index].line));
      Nan::Set(tick, Nan::New<String>("hitCount").ToLocalChecked(),
               Nan::New<Integer>(entries[index].hit_count));
...
void ProfileNode::setNodes_(v8::Isolate* isolate,
  ...
  for (int32_t index = 0; index < count; index++) {
    Nan::Set(children, index,
             Nan::New<Integer>(node->GetChild(index)->GetNodeId()));
  }
...
Local<Value> ProfileNode::New(v8::Isolate* isolate, const CpuProfileNode* node,
  ...
  // Trimmed to Zalo's legacy `binding.cpu` node shape (byte-for-byte field set):
  // functionName, url, scriptId, lineNumber, callUID, bailoutReason, hitCount,
  // children — dropping v8-profiler-next's extra columnNumber/id/lineTicks.
  Nan::Set(profile_node, Nan::New<String>("functionName").ToLocalChecked(),
           node->GetFunctionName());
  Nan::Set(profile_node, Nan::New<String>("url").ToLocalChecked(),
           node->GetScriptResourceName());
  Nan::Set(profile_node, Nan::New<String>("scriptId").ToLocalChecked(),
           Nan::New<Integer>(node->GetScriptId()));
  Nan::Set(profile_node, Nan::New<String>("lineNumber").ToLocalChecked(),
           Nan::New<Integer>(node->GetLineNumber()));
  // v8::CpuProfileNode::GetCallUid() was removed in V8 8; on Electron 22 we use
  // the stable per-node id (same integer domain) as the call identifier.
  Nan::Set(profile_node, Nan::New<String>("callUID").ToLocalChecked(),
           Nan::New<Integer>(node->GetNodeId()));
