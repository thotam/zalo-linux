...
| **zwalker** | ✅ chạy thật (RE Rust napi-rs) | Feature `cleanup.enable` gate OFF → app không tự gọi; dùng **startup probe** (`patch-zwalker-probe.js`) chạy đủ 5 hàm trên thư mục temp qua accessor đã-instrument. Phiên GUI E39 (2026-07-11): `LOAD zwalker → {5 hàm}` + tất cả `RET` đúng trong **mọi process** (browser/utility/renderer/node): `scanDirectory {fileNumber:4,size:750}` → `updateReferenceMessageId {2}` → `statUnmarkedFiles {2,450}` → `deleteHomelessFiles {2,450,failed:0}` (xoá thật) → `deleteEmptyFolders {deletedCount:3}`, `VERDICT markedSurvived=true homelessDeleted=true`, 0–4ms. Chỉ đụng `/tmp/zwalker-probe-*`. |
| **zcall** | ⬜ chưa kích hoạt | Không gọi call trong phiên |

**Kết luận:** **zjxl, zimage, sqlite3, file-utilities, file-utils, mp4thumb, zwalker — đã xác nhận chạy thật** trên E39 (mp4thumb verify 2026-07-11 qua màn zCloud: `generateThumbnail` local video → `RESOLVE true`; zwalker verify 2026-07-11 qua startup probe: 5 hàm `RET` đúng trong mọi process, `VERDICT` OK). Xem thêm bảng tổng quan RE ở [RE-ROADMAP.md](RE-ROADMAP.md).
...
| `zwalker` | Tự động: startup probe chạy 5 hàm trên thư mục temp → xem `ZWALKER-PROBE` trong log (không cần thao tác) |
